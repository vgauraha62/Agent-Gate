...
fn parsePolicy(value: std.json.Value, arena: *SecurityArena) !types.Policy {
    ...
    // Tool-aware conditions
    if (match_obj.get("tool")) |_| condition_count += 1;
    if (match_obj.get("tool_pattern")) |_| condition_count += 1;
    if (match_obj.get("command_pattern")) |_| condition_count += 1;
    if (match_obj.get("path_pattern")) |_| condition_count += 1;
    ...
    // === Tool-aware condition parsing ===

    // Parse tool condition (optional, string = exact tool name)
    if (match_obj.get("tool")) |v| {
        const tool = if (v == .string) v.string else return error.InvalidCondition;
        conditions[condition_idx] = types.Condition{ .tool = tool };
        condition_idx += 1;
    }

    // Parse tool_pattern condition (optional, string with wildcard support)
    if (match_obj.get("tool_pattern")) |v| {
        const pattern = if (v == .string) v.string else return error.InvalidCondition;
        conditions[condition_idx] = types.Condition{ .tool_pattern = pattern };
        condition_idx += 1;
    }

    // Parse command_pattern condition (optional, string = substring to match)
    if (match_obj.get("command_pattern")) |v| {
        const pattern = if (v == .string) v.string else return error.InvalidCondition;
        conditions[condition_idx] = types.Condition{ .command_pattern = pattern };
        condition_idx += 1;
    }

    // Parse path_pattern condition (optional, string with wildcard support)
    if (match_obj.get("path_pattern")) |v| {
        const pattern = if (v == .string) v.string else return error.InvalidCondition;
        conditions[condition_idx] = types.Condition{ .path_pattern = pattern };
        condition_idx += 1;
    }
...
fn parsePolicyComptime(value: std.json.Value) types.Policy {
    ...
    if (match_obj.get("tool") != null) condition_count += 1;
    if (match_obj.get("tool_pattern") != null) condition_count += 1;
    if (match_obj.get("command_pattern") != null) condition_count += 1;
    if (match_obj.get("path_pattern") != null) condition_count += 1;

    var conditions: [8]types.Condition = undefined;
    var condition_idx: usize = 0;
    ...
    // === Tool-aware condition parsing (comptime) ===
    if (match_obj.get("tool")) |v| {
        const tool = if (v == .string) v.string else @compileError("Invalid condition: tool must be a string");
        conditions[condition_idx] = types.Condition{ .tool = tool };
        condition_idx += 1;
    }

    if (match_obj.get("tool_pattern")) |v| {
        const pattern = if (v == .string) v.string else @compileError("Invalid condition: tool_pattern must be a string");
        conditions[condition_idx] = types.Condition{ .tool_pattern = pattern };
        condition_idx += 1;
    }

    if (match_obj.get("command_pattern")) |v| {
        const pattern = if (v == .string) v.string else @compileError("Invalid condition: command_pattern must be a string");
        conditions[condition_idx] = types.Condition{ .command_pattern = pattern };
        condition_idx += 1;
    }

    if (match_obj.get("path_pattern")) |v| {
        const pattern = if (v == .string) v.string else @compileError("Invalid condition: path_pattern must be a string");
        conditions[condition_idx] = types.Condition{ .path_pattern = pattern };
        condition_idx += 1;
    }
...
// ============================================================================
// Tool-aware condition parsing tests
// ============================================================================

test "parse policy with tool condition" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();
    ...
    try std.testing.expectEqual(@as(usize, 1), result.policies.len);
    try std.testing.expectEqual(@as(usize, 1), result.policies[0].conditions.len);
    try std.testing.expectEqual(types.Condition{ .tool = "bash" }, result.policies[0].conditions[0]);
}

test "parse policy with tool_pattern condition" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();
    ...
    const json =
        \\{"version": "1", "policies": [
        \\  {"id": "allow-read-tools", "effect": "allow", "match": {"tool_pattern": "read_*"}}
        \\]}
    ;

    const result = try parse(json, &arena);
    try std.testing.expectEqual(@as(usize, 1), result.policies.len);
    try std.testing.expectEqual(types.Condition{ .tool_pattern = "read_*" }, result.policies[0].conditions[0]);
}

test "parse policy with command_pattern condition" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();
    ...
    const json =
        \\{"version": "1", "policies": [
        \\  {"id": "block-rm-rf", "effect": "deny", "match": {"command_pattern": "rm -rf"}}
        \\]}
    ;

    const result = try parse(json, &arena);
    try std.testing.expectEqual(@as(usize, 1), result.policies.len);
    try std.testing.expectEqual(types.Condition{ .command_pattern = "rm -rf" }, result.policies[0].conditions[0]);
}

test "parse policy with path_pattern condition" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();
    ...
    const json =
        \\{"version": "1", "policies": [
        \\  {"id": "block-etc", "effect": "deny", "match": {"path_pattern": "/etc/*"}}
        \\]}
    ;

    const result = try parse(json, &arena);
    try std.testing.expectEqual(@as(usize, 1), result.policies.len);
    try std.testing.expectEqual(types.Condition{ .path_pattern = "/etc/*" }, result.policies[0].conditions[0]);
}

test "parse policy with multiple tool conditions" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();
    ...
    try std.testing.expectEqual(@as(usize, 2), result.policies[0].conditions.len);
    try std.testing.expectEqual(types.Condition{ .tool = "bash" }, result.policies[0].conditions[0]);
    try std.testing.expectEqual(types.Condition{ .command_pattern = "rm -rf" }, result.policies[0].conditions[1]);
}

test "parse policy with all tool conditions" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 65536);
    defer arena.deinit();
    ...
    const json =
        \\{"version": "1", "policies": [
        \\  {
        \\    "id": "complex-tool-rule",
        \\    "effect": "deny",
        \\    "match": {
        \\      "tool": "bash",
        \\      "tool_pattern": "read_*",
        \\      "command_pattern": "secret",
        \\      "path_pattern": "/etc/*"
        \\    }
        \\  }
        \\]}
    ...
    const result = try parse(json, &arena);
    try std.testing.expectEqual(@as(usize, 1), result.policies.len);
    try std.testing.expectEqual(@as(usize, 4), result.policies[0].conditions.len);
    try std.testing.expectEqual(types.Condition{ .tool = "bash" }, result.policies[0].conditions[0]);
    try std.testing.expectEqual(types.Condition{ .tool_pattern = "read_*" }, result.policies[0].conditions[1]);
    try std.testing.expectEqual(types.Condition{ .command_pattern = "secret" }, result.policies[0].conditions[2]);
    try std.testing.expectEqual(types.Condition{ .path_pattern = "/etc/*" }, result.policies[0].conditions[3]);
}

test "parse tool condition as number should fail" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();

    const json =
        \\{"version": "1", "policies": [{"id": "test", "effect": "allow", "match": {"tool": 123}}]}
    ;

    try std.testing.expectError(error.InvalidCondition, parse(json, &arena));
}

test "parse tool_pattern condition as number should fail" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();

    const json =
        \\{"version": "1", "policies": [{"id": "test", "effect": "allow", "match": {"tool_pattern": 456}}]}
    ;

    try std.testing.expectError(error.InvalidCondition, parse(json, &arena));
}

test "parse command_pattern condition as number should fail" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();

    const json =
        \\{"version": "1", "policies": [{"id": "test", "effect": "allow", "match": {"command_pattern": 789}}]}
    ;

    try std.testing.expectError(error.InvalidCondition, parse(json, &arena));
}

test "parse path_pattern condition as object should fail" {
    const gpa = std.testing.allocator;
    var arena = try SecurityArena.init(gpa, 4096);
    defer arena.deinit();

    const json =
        \\{"version": "1", "policies": [{"id": "test", "effect": "allow", "match": {"path_pattern": {"invalid": true}}}]}
    ;

    try std.testing.expectError(error.InvalidCondition, parse(json, &arena));
}

test "parse comptime policy with tool condition" {
    const json =
        \\{"version": "1", "policies": [
        \\  {"id": "block-rm-rf", "effect": "deny", "match": {"tool": "bash", "command_pattern": "rm -rf"}}
        \\]}
    ...
    try std.testing.expectEqual(@as(usize, 1), result.policies.len);
    try std.testing.expectEqual(@as(usize, 2), result.policies[0].conditions.len);
    try std.testing.expect(result.policies[0].conditions[0] == types.Condition{ .tool = "bash" });
}

test "parse comptime policy with all tool conditions" {
    const json =
        \\{"version": "1", "policies": [
        \\  {"id": "complex", "effect": "deny", "match": {"tool": "bash", "tool_pattern": "read_*", "command_pattern": "rm", "path_pattern": "/etc/*"}}
        \\]}
    ;
