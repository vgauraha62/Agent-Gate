...
const apikey_mod = @import("../apikey.zig");
...
pub const Server = struct {
    ...
    // Hosted API context (optional — null when not in hosted mode)
...
fn acceptConnections(self: *Self) void {
    ...
    fn handleRequestThread(self: *Self, client_fd: c_int) void {
        ...
        while (requests_served < max_requests_per_conn) {
            ...
            const is_hosted = self.hosted_ctx != null;
            const is_public_endpoint = std.mem.eql(u8, parsed.path, "/health") or
                ...
                std.mem.eql(u8, parsed.path, "/v1/agents") or
                // In hosted mode, /check and /v1/admin/* are accessible without SSL
                (is_hosted and (std.mem.eql(u8, parsed.path, "/check") or
                 std.mem.startsWith(u8, parsed.path, "/v1/admin/")));
            ...
            if (std.mem.eql(u8, parsed.path, "/health")) {
            ...
            } else if (std.mem.startsWith(u8, parsed.path, "/v1/admin/")) {
                // Admin endpoints (require admin key auth in hosted mode)
                if (self.hosted_ctx != null) {
                    self.handleAdminRequest(client_fd, parsed, buffer[0..@as(usize, @intCast(bytes_read))]) catch {
                        sendErrorResponse(client_fd, .internal_error, "Admin handler error");
                        break;
                    };
                } else {
                ...
                if (self.hosted_ctx) |ctx| {
                    ...
                    if (api_key_hdr) |key| {
                        const auth_result = ctx.authenticate(key);
                        if (auth_result == null) {
                            sendJsonResponse(client_fd, .unauthorized,
                    ...
                    // Non-hosted mode: existing behavior
    ...
    // ============================================================
    // Check Request Parser (Extended tool-aware format)
    // ============================================================
    ...
    fn parseCheckRequestFast(body: []const u8) CheckResult {
        ...
        var i: usize = 0;
        while (i + 5 < body.len) {
            // Detect if this is a tool request by looking for "tool" field
            if (!result.is_tool_request and body[i] == '"' and body[i+1] == 't' and
                body[i+2] == 'o' and body[i+3] == 'o' and body[i+4] == 'l' and body[i+5] == '"')
            {
                result.is_tool_request = true;
                var j = i + 6;
            ...
            if (body[i] == '"' and body[i+1] == 'c' and body[i+2] == 'o' and body[i+3] == 'm' and
                body[i+4] == 'm' and body[i+5] == 'a' and body[i+6] == 'n' and body[i+7] == 'd' and body[i+8] == '"')
            {
                var j = i + 9;
                while (j < body.len and body[j] != ':') j += 1;
                while (j < body.len and (body[j] == ':' or body[j] == ' ' or body[j] == '"')) j += 1;
    ...
    fn handleCheckRequest(self: *Self, client_fd: c_int, body: []const u8, agent_id: [32]u8) bool {
        ...
        const agent_hex = std.fmt.bytesToHex(agent_id, .lower)[0..];

        // Build request context for policy evaluation
        const ctx = if (check.is_tool_request)
        ...
        switch (decision.effect) {
            ...
            .deny => {
                // Determine the path/method for denial tracking
                const deny_path = if (check.is_tool_request) check.tool else check.path;
                const deny_method = if (check.is_tool_request) "POST" else check.method;
                ...
                // Build denial response with dynamic policy_id and reason
                var response_buf: [512]u8 = undefined;
                const response = std.fmt.bufPrint(&response_buf,
                    "{{\"allowed\":false,\"reason\":\"policy denied by: {s}\",\"policy_id\":\"{s}\"}}",
    ...
    // ============================================================
    // Admin API Endpoints (Hosted Mode)
    // ============================================================

    /// Extract a header value from the raw HTTP request buffer.
    fn extractHeaderValueRaw(buf: []const u8, header_name: []const u8) ?[]const u8 {
        // Skip request line
        var pos: usize = 0;
        while (pos < buf.len and buf[pos] != '\n') pos += 1;
        pos += 1; // Skip past the \n

        while (pos + header_name.len < buf.len) {
            // Check if this line starts with our header (case-insensitive)
            var match = true;
            for (header_name, 0..) |ch, i| {
                const b = if (pos + i < buf.len) buf[pos + i] else 0;
                if (std.ascii.toLower(b) != std.ascii.toLower(ch)) {
                    match = false;
                    break;
                }
            }
            if (match and buf[pos + header_name.len] == ':') {
                // Skip header name and colon
                var j = pos + header_name.len + 1;
                // Skip whitespace
                while (j < buf.len and (buf[j] == ' ' or buf[j] == '\t')) j += 1;
                const value_start = j;
                // Find end of line
                while (j < buf.len and buf[j] != '\r' and buf[j] != '\n') j += 1;
                return buf[value_start..j];
            }
            // Skip to next line
            while (pos < buf.len and buf[pos] != '\n') pos += 1;
            pos += 1;
    ...
    fn handleAdminRequest(self: *Self, client_fd: c_int, parsed: ParsedRequest, raw_buf: []const u8) !void {
        const ctx = self.hosted_ctx orelse {
            sendNotFoundResponse(client_fd);
            return;
        };

        // Extract API key from X-API-Key header or Authorization header
        const api_key = extractHeaderValueRaw(raw_buf, "X-API-Key") orelse
            extractHeaderValueRaw(raw_buf, "Authorization");

        // Authenticate
        const auth = ctx.authenticate(api_key) orelse {
            const err = try hosted.jsonError(self.allocator, 401, "missing or invalid API key");
            defer self.allocator.free(err);
            sendJsonResponse(client_fd, .unauthorized, err);
            return;
        };

        // Only admin key can access admin endpoints
        if (auth != .admin) {
            const err = try hosted.jsonError(self.allocator, 403, "admin access required");
            defer self.allocator.free(err);
            sendJsonResponse(client_fd, .forbidden, err);
            return;
        }

        // Route to appropriate handler
        if (std.mem.eql(u8, parsed.method, "GET") and std.mem.eql(u8, parsed.path, "/v1/admin/keys")) {
            // List all API keys
            const json = try hosted.buildKeysListJson(ctx, self.allocator);
            defer self.allocator.free(json);
            sendJsonResponse(client_fd, .ok, json);
        } else if (std.mem.eql(u8, parsed.method, "POST") and std.mem.eql(u8, parsed.path, "/v1/admin/keys")) {
            // Create a new API key
            self.handleAdminCreateKey(client_fd, ctx, parsed.body);
        } else if (std.mem.eql(u8, parsed.method, "DELETE") and std.mem.eql(u8, parsed.path, "/v1/admin/keys")) {
            // Revoke an API key
            try self.handleAdminRevokeKey(client_fd, ctx, parsed.query);
        } else if (std.mem.eql(u8, parsed.method, "GET") and std.mem.eql(u8, parsed.path, "/v1/admin/usage")) {
            // Get usage stats
            const json = try hosted.buildUsageJson(ctx, self.allocator);
            defer self.allocator.free(json);
            sendJsonResponse(client_fd, .ok, json);
        } else if (std.mem.eql(u8, parsed.method, "GET") and std.mem.eql(u8, parsed.path, "/v1/admin/stats")) {
            // Get server stats
            const json = try hosted.buildServerStatsJson(ctx, self.allocator);
            defer self.allocator.free(json);
            sendJsonResponse(client_fd, .ok, json);
        } else {
            sendNotFoundResponse(client_fd);
    ...
    fn handleAdminCreateKey(self: *Self, client_fd: c_int, ctx: *hosted.HostedContext, body: []const u8) void {
        // Parse request body (simple JSON parsing without full parser)
        var name: []const u8 = "default";
        var rate_limit: u32 = ctx.config.default_rate_limit;
        var expires_at: i64 = 0;
        var notes: []const u8 = "";

        if (body.len > 0) {
            if (extractJsonStringField(body, "name")) |v| name = v;
            if (extractJsonNumberField(body, "rate_limit")) |v| {
                rate_limit = @min(@as(u32, @intCast(v)), ctx.config.max_rate_limit);
            }
            if (extractJsonNumberField(body, "expires_at")) |v| expires_at = @as(i64, @intCast(v));
            if (extractJsonStringField(body, "notes")) |v| notes = v;
        }

        // Create the key
        const full_key = ctx.key_store.createKey(name, rate_limit, expires_at, notes) catch |err| {
            const msg = if (err == error.MaxApiKeysReached) "max API keys reached" else "failed to create key";
            const resp = hosted.jsonError(self.allocator, 500, msg) catch return;
            defer self.allocator.free(resp);
            sendJsonResponse(client_fd, .internal_error, resp);
            return;
        };
        ...
        const response = std.fmt.bufPrint(&buf,
            \\{{"success":true,"key":"{s}","key_prefix":"{s}","name":"{s}","rate_limit":{},"expires_at":{},"notes":"{s}"}}
        , .{ full_key, full_key[0..@min(full_key.len, apikey_mod.KEY_PREFIX.len + 8)], name, rate_limit, expires_at, notes }
        ) catch {
            sendErrorResponse(client_fd, .internal_error, "response too large");
            return;
        };
        sendJsonResponse(client_fd, .ok, response);
    }

    /// DELETE /v1/admin/keys?prefix=ag_abc123 — revoke a key.
    fn handleAdminRevokeKey(self: *Self, client_fd: c_int, ctx: *hosted.HostedContext, query: []const u8) !void {
        ...
        if (prefix.len == 0) {
            const err = try hosted.jsonError(self.allocator, 400, "missing 'prefix' query parameter");
            defer self.allocator.free(err);
            sendJsonResponse(client_fd, .bad_request, err);
            return;
        }

        const revoked = ctx.key_store.revokeKey(prefix) catch {
            sendErrorResponse(client_fd, .internal_error, "failed to revoke key");
            return;
        };

        if (revoked) {
            const msg = try hosted.jsonSuccess(self.allocator, "key revoked");
            defer self.allocator.free(msg);
            sendJsonResponse(client_fd, .ok, msg);
        } else {
            const err = try hosted.jsonError(self.allocator, 404, "key not found or already revoked");
            defer self.allocator.free(err);
            sendJsonResponse(client_fd, .not_found, err);
        }
    }
...
// ============================================================================
// Simple JSON field extraction (no alloc, no full parser)
// ============================================================================

/// Extract a string field value from a JSON object body.
/// Does NOT handle escaped quotes or nested objects.
pub fn extractJsonStringField(body: []const u8, field: []const u8) ?[]const u8 {
    const search_buf = std.fmt.allocPrint(std.heap.page_allocator, "\"{s}\"", .{field}) catch return null;
    defer std.heap.page_allocator.free(search_buf);
    const search = search_buf;

    var i: usize = 0;
    while (i + search.len < body.len) : (i += 1) {
        if (std.mem.eql(u8, body[i..][0..search.len], search)) {
            // Found field, find colon
            var j = i + search.len;
            while (j < body.len and body[j] != ':') j += 1;
            // Skip colon and whitespace
            j += 1;
            ...
            if (j < body.len and body[j] == '"') {
                ...
                const start = j;
                while (j < body.len and body[j] != '"') j += 1;
                if (j > start) return body[start..j];
            }
            return null;
        }
    }
...
/// Extract a numeric field value from a JSON object body.
pub fn extractJsonNumberField(body: []const u8, field: []const u8) ?i64 {
    const search_buf = std.fmt.allocPrint(std.heap.page_allocator, "\"{s}\"", .{field}) catch return null;
    defer std.heap.page_allocator.free(search_buf);
    const search = search_buf;
    ...
    while (i + search.len < body.len) : (i += 1) {
        if (std.mem.eql(u8, body[i..][0..search.len], search)) {
            ...
            if (j > start) {
                return std.fmt.parseInt(i64, body[start..j], 10) catch null;
            }
            return null;
        }
    }
